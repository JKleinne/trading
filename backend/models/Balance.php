...
class Balance extends Model
{
    ...
    public function createBalance($currency_code, $balance)
    {
        $stmt = $this->_connection->prepare("INSERT INTO Wallet(:user_id, :currency_code, :balance)");
        $stmt->execute(['user_id'=>$_SESSION["user_id"], 'currency_code'=>$currency_code, 'balance'=>$balance]);
    }

    public function withdraw($amount, $currency_code)
    {
        $stmt = $this->_connection->prepare("SELECT balance FROM Wallet WHERE user_id = :user_id AND currency_code = :currency_code");
        $currentVal = $stmt->execute(['user_id'=>$_SESSION["user_id"], 'currency_code'=>$currency_code]);
        $newBalance = $currentVal + $amount;

        $stmt = $this->_connection->prepare("UPDATE Wallet SET balance = :balance WHERE user_id = :user_id AND currency_code = :currency_code");
        $stmt->execute(['balance'=>$newBalance, 'user_id'=>$_SESSION["user_id"], 'currency_code'=>$currency_code]);
    }

    public function deposit($amount, $currency_code)
    {
        $stmt = $this->_connection->prepare("SELECT balance FROM Wallet WHERE user_id = :user_id AND currency_code = :currency_code");
        $currentVal = $stmt->execute(['user_id'=>$_SESSION["user_id"], 'currency_code'=>$currency_code]);
        $newBalance = $currentVal - $amount;
    ...
    public function buy($pay_amount, $pay_currency_code, $buy_amount, $buy_currency_code)
    {
        $stmt = $this->_connection->prepare("SELECT balance FROM Wallet WHERE user_id = :user_id AND currency_code = :currency_code");
    ...
    public function getPorfolioValue($user_id, $currency_code)
    {
        $stmt = $this->_connection->prepare("SELECT balance FROM Wallet WHERE user_id = :user_id AND currency_code = :currency_code");
        return $stmt->execute(['user_id'=>$user_id, 'currency_code'=>$currency_code])->fetch();
    }
}